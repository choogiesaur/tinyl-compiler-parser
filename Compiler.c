...
{
    int reg;
        //i think this works, double check
	if (!is_identifier(token)) {
		ERROR("Expected variable name, single letter from a...e\n");
		exit(EXIT_FAILURE);
	}
	reg = next_register();
   	CodeGen(LOAD, reg, token, EMPTY_FIELD); // might have to change!!! - changed from LOADI to LOAD. now, what to do with 3rd arg, token
	next_token();
	return reg;
}
...
static void read()
{
	...
	CodeGen(READ, token, EMPTY_FIELD, EMPTY_FIELD); // might need to change
	next_token();
}
...
static void stmt()
{
	switch (token) {
		...
		case '?':	
			return read();
		default:
			ERROR("Expected assignment, read, or print statement", token);
			exit(EXIT_FAILURE);
	}
}
...
static void morestmts()
{
	if (token == ';' ) {
		...
		ERROR("Expected more statements or end of file");
		exit(EXIT_FAILURE);
	}
	return;
}
...
static void program()
{
	...
	//maybe added condition for starting. only start if current is not '.'?
	stmtlist();
	if (token != '.') {
